...
export function getBillingCycle(card, referenceDate = new Date()) {
  ...
  if (day <= closing) {
    ...
    cycleEnd = new Date(year, month, closing, 23, 59, 59, 999);
  } else {
    ...
    cycleEnd = new Date(year, month + 1, closing, 23, 59, 59, 999);
  }
...
/* ----------------------------------------------------------
   ...
   Regra (consistente com getBillingCycle):
     - Se o dia da compra > closing_day → cai no próximo ciclo
     - Se o dia da compra <= closing_day → fica no ciclo atual
   ---------------------------------------------------------- */
...
export function generateInstallments(card, purchaseDate, totalAmount, count) {
  ...
  // Distribui o valor em centavos para evitar diferenças de arredondamento
  // (ex.: 100 / 3 = 33.33... -> 33.33 + 33.33 + 33.34 = 100.00)
