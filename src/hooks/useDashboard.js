...
export function useDashboard() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      // Saldo disponível real = soma do current_balance das contas
      // (mantido atomicamente pela RPC adjust_account_balance).
      const totalBalance =
        myAccounts?.reduce(
      ...
      // 2. Despesas (EXPENSE) — só saídas da conta (débito/pix/pagamento de
      // fatura). Compras no cartão entram na fatura, não no fluxo do mês.
