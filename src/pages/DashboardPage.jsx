...
import { 
  ...
  Plus, CreditCard, ChevronRight, Sun, Moon, Search, RefreshCw, TrendingUp 
} from 'lucide-react'
...
export default function DashboardPage() {
  ...
  // Recebe dados atualizados (com totalInvested separado)
  ...
  return (
    <div className="pb-32 lg:pb-0 space-y-6 max-w-5xl mx-auto animate-in fade-in duration-500 pt-4">
      ...
      {/* HEADER REMOVIDO (Já está no AppLayout) */}
      ...
      <div className="relative w-full bg-slate-900 dark:bg-black rounded-[2rem] p-6 text-white shadow-2xl shadow-slate-900/20 dark:shadow-black/50 overflow-hidden flex flex-col justify-between transition-all hover:scale-[1.01] duration-500">
        ...
        <div className="relative z-10 flex flex-col gap-6">
          ...
          {/* Linha 1: Saldo Disponível (Caixa) */}
      ...
      {/* AÇÕES RÁPIDAS (Navegam para as páginas) */}
      <div className="flex justify-between px-2 sm:px-4">
        {[
          { 
            ...
            action: () => navigate('/transactions/new-income') // <--- Rota de Receita
          },
          { 
            ...
            action: () => navigate('/transactions/new-expense') // <--- Rota de Despesa
          },
          { 
            icon: Plus, 
            label: 'Nova Conta', 
            color: 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30 hover:bg-indigo-700', 
            action: () => navigate('/accounts/new') // <--- Rota de Conta
          },
        ].map((item, idx) => (
          <button 
            key={idx} 
