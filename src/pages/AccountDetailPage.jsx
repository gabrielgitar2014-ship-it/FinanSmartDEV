...
export default function AccountDetailPage() {
  ...
  return (
    <div className="pt-16 pb-28 lg:pb-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
      
      {/* HEADER FIXO */}
      <div className="fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-2 py-4 bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <button
          onClick={() => navigate('/accounts')}
          className="p-2 bg-white dark:bg-slate-800 rounded-full text-slate-500 hover:text-indigo-600 shadow-sm transition-colors"
        >
          <ChevronLeft size={20} />
        </button>
      ...
      <div className="px-2">
        <div
          className="relative w-full min-h-[200px] rounded-[2rem] p-6 text-white shadow-2xl overflow-hidden flex flex-col justify-between transition-all"
          style={{
            backgroundColor: bankVisuals.color,
            color: bankVisuals.textColor || '#fff'
          }}
        >
          ...
          <div className="relative z-10 flex justify-between items-start">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-white rounded-2xl p-1 shadow-md">
                <img
                  src={bankVisuals.logo || '/logos/vite.svg'}
                  alt={bankVisuals.name}
                  className="w-full h-full object-contain"
                  onError={(e) => {
                    e.target.onerror = null
                    e.target.src = '/logos/vite.svg'
                  }}
                />
              </div>
              <div>
            ...
            {(account.type === 'investment_hub' ||
              ...
              <div className="pt-4 border-t border-white/20 flex items-center gap-2">
                ...
                <span className="text-sm font-mono font-bold">
                  {formatCurrency(account.investment_balance)}
                </span>
              </div>
            )}
          </div>
      ...
      <div className="px-4 mt-6 mb-4">
        <div className="flex p-1 bg-slate-200 dark:bg-slate-900 rounded-xl">
          {[
            { id: 'overview', label: 'Cartões', icon: CreditCard },
            { id: 'pix', label: 'Pix', icon: Key },
      ...
      <div className="px-4 pb-20">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs bg-slate-200 dark:bg-slate-800 px-2 py-1 rounded-full text-slate-600 dark:text-slate-300">
                {creditCards.length}
              </span>
            </div>
              ...
              {creditCards.map((card) => {
                ...
                return (
                  <div
                    ...
                    className="min-w-[300px] h-[200px] bg-slate-900 text-white rounded-2xl p-5 shadow-lg flex flex-col justify-between relative overflow-hidden snap-center group shrink-0 cursor-pointer"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent pointer-events-none" />

                    <div className="flex justify-between items-start relative z-10">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-5 bg-white/20 rounded flex items-center justify-center text-[8px] font-bold tracking-widest">
                          CHIP
                        </div>
                        <span className="text-xs opacity-70">
                          Final {card.last_4_digits || '****'}
                        </span>
                      </div>
                      <button
                        onClick={(e) => {
                        ...
                        className="p-1.5 bg-white/10 rounded-full hover:bg-red-500 hover:text-white text-slate-300 transition-colors"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
                    ...
                    <div className="flex justify_between items-end relative z-10">
                      <div>
        ...
        {activeTab === 'pix' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setIsAddingPix(!isAddingPix)}
                ...
                >
                  <div className="bg-indigo-50 dark:bg-indigo-900/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 mb-4 flex gap-2">
                    <select
                      value={newPix.type}
                      onChange={(e) =>
                        setNewPix({ ...newPix, type: e.target.value })
                      }
                      className="bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 rounded-lg text-xs font-bold outline-none p-2"
                    >
                    ...
                    <input
                      value={newPix.key}
                      onChange={(e) =>
                        setNewPix({ ...newPix, key: e.target.value })
                      }
                      placeholder="Chave..."
                      className="flex-1 bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 rounded-lg p-2 text-sm outline-none"
                    />
                    <button
                      onClick={handleAddPix}
                      disabled={actionLoading}
                      className="bg-indigo-600 text_white p-2 rounded-lg hover:bg-indigo-700"
                    >
                      {actionLoading ? (
            ...
            <div className="space-y-2">
              {(account.pix_keys || []).length === 0 ? (
                <p className="text-center text-slate-400 py-8 text-sm">
                  Nenhuma chave cadastrada.
                </p>
              ) : (
                (account.pix_keys || []).map((pix, idx) => (
                  <div
                    key={idx}
        ...
        {activeTab === 'invest' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
            ...
            {!isAddingInvest ? (
              <button
                onClick={() => setIsAddingInvest(true)}
                className="w-full py-3 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl text-slate-500 font-bold flex items-center justify-center gap-2 hover:border-emerald-500 hover:text-emerald-600 transition-all"
              >
                <Plus size={20} /> Registrar Aporte / Retirada
              </button>
            ) : (
              <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-lg">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setIsAddingInvest(false)}>
                ...
                <div className="relative mb-4">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 pl-10 font-mono text-lg font-bold outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>

                <p className="text-xs text-slate-400 mb-4">
                  Para retiradas, use valor negativo (ex: -500).
                </p>
