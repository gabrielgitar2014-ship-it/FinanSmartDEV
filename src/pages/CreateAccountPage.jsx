...
export default function CreateAccountPage() {
  ...
  const [investDisplay, setInvestDisplay] = useState('') 
  
  // Módulo Cartões (Múltiplos)
  const [hasCredit, setHasCredit] = useState(false)
  const [cards, setCards] = useState([]) 
  // Estado temporário para o input do cartão
  ...
  // --- LÓGICA CARTÕES (Adicionar à lista) ---
  const handleAddCardToList = () => {
    // Valida os campos do 'newCard'
    ...
    setNewCard({ name: '', limit: '', closing: '', due: '' }) // Limpa form
    setIsAddingCard(false) // Fecha form
  }
  ...
  const handleCreate = async () => {
    ...
    // CORREÇÃO DO ERRO: Verifica se tem cartões NA LISTA, não nos inputs
    if (hasCredit && cards.length === 0) {
        return toast.error('Você ativou Cartão de Crédito, mas não adicionou nenhum. Adicione um cartão à lista ou desative a opção.')
    }
    ...
    try {
      ...
      const { data: accountData, error: accError } = await supabase
        ...
        .insert({
          ...
          initial_balance: checkingValue,
          current_balance: checkingValue,
          investment_balance: investValue,
      ...
      if (hasCredit && cards.length > 0) {
        const cardsToInsert = cards.map(c => {
            // Parsing seguro do limite do cartão
            const limitVal = typeof c.limit === 'string' 
                ? parseFloat(c.limit.replace(/\./g, '').replace(',', '.')) 
                : c.limit
            ...
            return {
                ...
                available_limit: limitVal, // Inicia com limite total disponível
                closing_day: parseInt(c.closing),
                due_day: parseInt(c.due),
                last_4_digits: '****'
            }
        })

        const { error: cardError } = await supabase.from('credit_cards').insert(cardsToInsert)
            ...
            >
              ...
              <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <div className="relative">
                    ...
                    <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg py-1.5 pl-7 pr-2 text-xs font-bold outline-none appearance-none">
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
                                ...
                                {cards.map(card => (
                                    <div key={card.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                                        <div>
                                            <p className="font-bold text-sm text-slate-900 dark:text-white">{card.name}</p>
                                            <p className="text-xs text-slate-500">R$ {card.limit} • Fatura dia {card.due}</p>
                                        </div>
                                ...
                                {isAddingCard ? (
                                    <div className="bg-indigo-50 dark:bg-indigo-900/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 space-y-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs font-bold text-indigo-600 uppercase">Novo Cartão</span>
                                            <button onClick={() => setIsAddingCard(false)}><X size={16} className="text-indigo-400" /></button>
                                        </div>
                                        <input placeholder="Apelido (ex: Black)" value={newCard.name} onChange={e => setNewCard({...newCard, name: e.target.value})} className="w-full bg-white dark:bg-slate-900 p-2 rounded-lg text-sm outline-none" />
                                        
                                        {/* Input de Limite com Máscara */}
                                        <input 
                                            ...
                                            onChange={e => {
                                                ...
                                                if(!val) { setNewCard({...newCard, limit: ''}); return }
                                                const num = parseFloat(val)/100
                                                setNewCard({...newCard, limit: num.toLocaleString('pt-BR', {minimumFractionDigits: 2})})
                                            }} 
                                            className="w-full bg-white dark:bg-slate-900 p-2 rounded-lg text-sm outline-none font-bold" 
                                        />
                                        
                                        <div className="grid grid-cols-2 gap-2">
                                            <input type="number" placeholder="Dia Fechamento" value={newCard.closing} onChange={e => setNewCard({...newCard, closing: e.target.value})} className="w-full bg-white dark:bg-slate-900 p-2 rounded-lg text-sm outline-none text-center" />
                                            <input type="number" placeholder="Dia Vencimento" value={newCard.due} onChange={e => setNewCard({...newCard, due: e.target.value})} className="w-full bg-white dark:bg-slate-900 p-2 rounded-lg text-sm outline-none text-center" />
                                        </div>
                                        <button onClick={handleAddCardToList} className="w-full bg-indigo-600 text-white py-2 rounded-lg font-bold text-sm">Salvar Cartão</button>
                                    </div>
                                ) : (
                                    <button onClick={() => setIsAddingCard(true)} className="w-full py-3 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl text-slate-500 font-bold text-sm hover:border-indigo-500 hover:text-indigo-500 transition-colors flex items-center justify-center gap-2">
