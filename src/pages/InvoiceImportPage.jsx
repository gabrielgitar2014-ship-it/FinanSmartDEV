...
import { 
  ...
  Edit3, Calendar, DollarSign, AlertCircle, History, FastForward 
} from 'lucide-react';
...
export default function InvoiceImportPage() {
  ...
  // Lista de Transações Confirmadas
  ...
  const expandTransaction = (tx) => {
    // Se não tem parcela ou expansão desligada, retorna o item original
    ...
    try {
      ...
      // Tenta parsear a data (Formato esperado dd/MM)
      // Assume o ano atual se não vier na string
      const currentYear = new Date().getFullYear();
      let dateObj = parse(tx.date, 'dd/MM', new Date());
      
      // Se a data for inválida, tenta dd/MM/yyyy ou ignora expansão
      if (!isValid(dateObj)) {
      ...
      for (let i = 1; i <= total; i++) {
        // Calcula o deslocamento de meses em relação à parcela ATUAL
        // Ex: Se estou na 10 (current), a parcela 1 (i) é -9 meses.
        const monthOffset = i - current;
        const newDate = addMonths(dateObj, monthOffset);
        
        let status = 'pending'; // Padrão (Atual)
        let tag = 'Atual';
        
        if (i < current) {
            status = 'consolidated'; // Passado
            tag = 'Passado';
        } else if (i > current) {
            status = 'scheduled'; // Futuro
            tag = 'Futuro';
        }

        expandedItems.push({
          ...tx,
          id: `${tx.original_id || Date.now()}_inst_${i}`, // ID Único Composto
          date: format(newDate, 'dd/MM/yyyy'),
          description: `${tx.description} (${i}/${total})`,
          value: tx.value, // Mantém o valor da parcela
          installment: `${i}/${total}`,
          status: status, // Usado para UI (cor diferente)
          tag: tag
        });
      }
  ...
  // Função para processar novos itens (do Python ou Manual) e aplicar expansão
  const addTransactions = (newItems) => {
    let finalItems = [];
    
    newItems.forEach(item => {
        // Se tiver parcela, expande em várias transações
        if (item.installment) {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      if (data.visual_data.auto_transactions) {
        // Aplica a lógica de expansão nos itens automáticos também!
  ...
  const processManualSelection = async (boxRect, pageNum) => {
    ...
    // ... (Lógica de cálculo de coordenadas idêntica ao anterior) ...
    ...
    try {
        ...
        if (data.transactions?.length > 0) {
            addTransactions(data.transactions); // Usa a função inteligente
  ...
  // ... (Gestores de Eventos de Mouse/Touch mantidos iguais ao anterior para brevidade) ...
  ...
  // Totalizador (Soma tudo ou só o mês atual? Normalmente soma tudo que vai ser importado)
      ...
      <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 font-sans animate-in slide-in-from-right-8 duration-500">
                ...
                >
                  ...
                  <div className="flex md:flex-col items-center md:items-start gap-2 min-w-[100px]">
                    ...
                    <div className="flex gap-1">
                        ...
                        {tx.tag && tx.tag !== 'Atual' && (
                            <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold flex items-center gap-1 ${tx.tag === 'Passado' ? 'text-slate-500 bg-slate-200' : 'text-indigo-600 bg-indigo-100'}`}>
        ...
        <div className="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-4 md:p-6 sticky bottom-0 z-20">
          <div className="max-w-3xl mx-auto">
            <button 
              onClick={() => alert("Lógica de salvar no Supabase aqui!")}
              ...
              {confirmedTransactions
                ...
                .map(tx => (
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      // Remove todas as parcelas relacionadas se for um grupo expandido? 
                      // Por enquanto remove só o item clicado visualmente.
            ...
            <div key={tx.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm flex items-center justify-between gap-3">
              <div className="flex flex-col min-w-0 gap-1">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">{tx.date || 'S/D'}</span>
                  {tx.installment && <span className="text-[10px] font-bold text-amber-700 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded">{tx.installment}</span>}
                </div>
