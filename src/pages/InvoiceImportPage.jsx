...
import { expandInvoiceInstallments } from '../utils/invoiceCalculator';
...
const FuturisticLoader = () => (
  <div className="fixed inset-0 z-[100] bg-white-300/95 backdrop-blur-xl flex flex-col items-center justify-center overflow-hidden">
...
export default function SmartInvoiceAuditor() {
  ...
  const handleFinalSave = () => {
    if (!selectedAccount) {
        toast.warning("Selecione a Conta/Cartão para continuar.");
        return;
    }
    if (!invoiceDate) {
        toast.warning("Selecione o Mês de Referência.");
        return;
    }
    ...
    try {
      // 1. Gera os dados expandidos localmente (JavaScript puro)
      const expandedData = expandInvoiceInstallments(
        confirmedTransactions,
        invoiceDate,
        selectedAccount
      );

      // 2. Navega para a página de revisão enviando os dados
      navigate('/invoice-review', { 
        state: { 
          transactions: expandedData,
      ...
      console.error(err);
      toast.error("Erro ao preparar dados para revisão.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!visualData) {
   return (
    <div className="min-h-screen flex flex-col bg-slate-100 font-sans selection:bg-indigo-100 relative overflow-hidden">
      ...
      {/* CORREÇÃO DO TOASTER:
          1. position="top-center": Para não brigar com o footer/gaveta lá embaixo.
          2. style={{ zIndex: 99999 }}: Garante que flutue acima de TUDO (Modais, Gavetas, Navbars).
      */}
      <Toaster 
        position="top-center" 
        richColors 
        ...
        style={{ zIndex: 99999 }} // Força bruta no container
      />
